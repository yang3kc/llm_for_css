...
You can run the full script ([senti_basic.py](https://github.com/yang3kc/llm_for_css/blob/main/basics/senti_basic.py)) with the following command:

```bash
uv run senti_basic.py
```

??? example "Full script: basics/senti_basic.py"

    ```python
    --8<-- "basics/senti_basic.py"
    ```
...
See [structured output](structured_output.md) for details.

For issue 2, there are two options:

1. If you need the results immediately, you can consider using the async programming to accelerate the querying process. See [async programming](async_programming.md) for details.
1. If you are not in a rush, you can use the batch API to process large amounts of data with reduced cost. See [batch processing](batch_processing.md) for details.
