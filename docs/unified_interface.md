...
!!! note
    ...
    This section needs updates; the code targets older SDK versions and models.

In this section, I provide a unified interface for querying different models.
...
??? example "Full script: unified_interface/api_factory.py"

    ```python
    --8<-- "unified_interface/api_factory.py"
    ```

??? example "Full script: unified_interface/query_llm.py"

    ```python
    --8<-- "unified_interface/query_llm.py"
    ```
